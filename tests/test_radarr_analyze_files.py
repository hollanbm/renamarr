...
class TestAnalyzeFiles:
    ...
    def test_process_logs_success_when_rescan_succeeds(
        ...
        radarr_cli = RadarrCli("test.tld", "test-api-key")
        mocker.patch(
            "renamarr.radarr.services.analyze_files.get_observability",
        ...
        send_command = mocker.patch.object(
            radarr_cli, "_sendCommand", return_value=dict(id=1)
        )
        ...
        sleep = mocker.patch("renamarr.radarr.services.analyze_files.sleep")

        AnalyzeFiles(radarr_cli, name="movies").process()
    ...
    def test_process_logs_failure_and_records_timeout_when_rescan_times_out(
        ...
        sleep = mocker.patch("renamarr.radarr.services.analyze_files.sleep")
        mocker.patch(
            "renamarr.radarr.services.analyze_files.time.monotonic",
        ...
        mock_loguru_error.assert_called_once_with(
            "Timed out waiting for Radarr analyze files command 1 after 300 seconds"
        )
        mock_loguru_info.assert_has_calls(
            [
                call("Initiated disk scan of library"),
